...
function tank_start(canvas,mapdata){
	...
	// --------- init entity ----------
	//        myTank
	ent_myTank = new Tank(res_pictures.mytank,4,4,SOUTH,3);
	ent_myTank2 = new Tank(res_pictures.mytank2,30,4,NORTH,3);
	//ent_myTank2.driver = new Driver();
...
function Bullet(pic,x,y,dir,src){
	...
	this.fly = function(){
		...
		switch(this.direction){
			case NORTH:
			case SOUTH:
				if (collisionTest(this.x, this.y) ||
				collisionTest(this.x-1, this.y)){
					explodeAt(this,Math.floor(this.x),Math.floor(this.y));
